...
/* Baseline NIF uses headers under rapidxml_base/ so c_src/*.hpp can diverge for experiments. */
#include "rapidxml_base/rapidxml.hpp"
#include "rapidxml_base/rapidxml_print.hpp"
#include <erl_nif.h>
...
static int load(ErlNifEnv *env, void **, ERL_NIF_TERM) {
  parser_type = enif_open_resource_type(
      env, "exml_nif_base", "parser", &delete_parser, ERL_NIF_RT_CREATE, nullptr);
...
ERL_NIF_INIT(exml_nif_base, nif_funcs, &load, nullptr, nullptr, &unload)
